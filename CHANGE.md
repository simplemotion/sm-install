...
|---------|------|------|--------|-------|
| (auto) | &mdash; | 2026-08-19 | Greg Gowans | **Key the store by release tag and keep every version.** The binary now lands at `share/<channel>/<package>/<tag>` instead of overwriting a single file per channel, and nothing is pruned — `bin/<package>` is the symlink that says which version is live, so rolling back is re-pointing a link rather than fetching an older build. Disk is the price and it is unbounded by design, roughly the size of the binary per retained version per channel; deleting a version directory by hand is safe as long as it is not the symlink's target. **The migration is the careful part.** The previous layout kept the binary at exactly the path that now has to be a directory, and `mkdir` will not do that over a file. It is moved aside rather than deleted outright, because between removing it and writing the new copy the symlink would be dangling and a failure in that window would leave the command missing altogether; the displaced file is removed only once the new one is in place. Verified against a reproduction of the real on-disk shape: file becomes directory, symlink follows, no `.superseded` temp left behind. **This also fixes the fast path, which had never once fired on the release channel.** It compared `sm-welcome -V` against the channel's tag, and those can never be equal there — the binary stamps itself from the develop tag it was built at (`0.1.0-develop-229`) while the release channel's tag is the bare GA (`0.1.0`) — so every run re-downloaded a binary it already had. Now the store is keyed BY tag the question is simply whether that file exists, which cannot drift from what the binary reports because it no longer asks. Confirmed by two consecutive runs: the second skipped the download entirely and the step total dropped from 32 to 22, which is the variable budget doing its job as well. |
| v0.1.2-develop-032 | a0f4867 | 2026-08-19 05:41 UTC | Greg Gowans | **Bring the bootstrap's own phases into the same format as everything after them, and fix a step counter that had drifted past its own total.** Prerequisites printed uncoloured ASCII `[v]`/`[*]`/`[x]` with no step numbers while the phases either side used coloured `[✓]` and `[NN/TT]`, and its section rule was 56 wide against the 36 used by the download phase and the binary — so the first thing a new starter saw looked like a different program from the rest of the run. Prerequisites now numbers its three steps, `confirm_section` draws the same width-36 dim rule with a bold title that `phase_header` draws on the Rust side, and the section titles drop the *Section N of 3* prefix because the counter now carries progress. **The totals were wrong, not merely inconsistent.** `OFFSET=5` and `TOTAL=23` were hardcoded on the stated belief that the binary had 18 steps. It has 19, so the last step rendered `[24/23]` — a counter past its own total — while the comment described it as *one short*, documenting a state that had stopped being true. The budget is now computed from named constants at runtime and, because it must be known before the first line prints, the fast-path resolution moved above Prerequisites: a counter that cannot say how many steps there are until halfway through is not a counter. Verified across all four skip/onboard combinations on both platforms — the binary's last step lands exactly on the total in every one. **`sm-install.sh` gained a step offset** so its five steps continue the count instead of restarting; both passes previously printed `[01..05]` and the binary then resumed from a number neither had reached. **It also stops drawing its own Download header when the bootstrap already opened that phase** — the two passes gave one phase three rules and made it look like three. **Found while testing, NOT fixed here: the fast path is dead on the release channel.** It compares the binary's stamped version (`0.1.0-develop-229`) against the channel's latest tag (`0.1.0`); they can never be equal, so every run re-downloads sm-welcome and sm-onboard. That is an install-behaviour bug, not a formatting one, and it deserves its own change rather than riding along in this one. |
| v0.1.2-develop-031 | cf231be | 2026-08-19 03:38 UTC | Greg Gowans | **`preview` is staff-only: `ent:sm-freelance` came off it.** Every channel but `release` now has the same two teams, `ent:sm-executive` and `ent:sm-employees`, so the README names them once instead of maintaining a per-channel table that was already wrong. **Why a candidate channel is staff-only:** the ladders fork at preview, so a staff-only package *transits* it on the way to `private` — reading preview means reading every internal product's candidate builds, not just early access to public ones. That reasoning is now in the README, because otherwise the obvious future change is to widen preview to freelance or customers on the grounds that it only holds candidates. **Nothing here is a code path change** — `preview` was already treated as non-public in the channel case and the credentials branch, and the unauthenticated message already avoided naming cohorts. What changed is a stale comment calling it `staff+freelance` and three README lines that promised access the grants no longer give. Verified the installer still behaves: unauthenticated `--channel preview` gives the credentials message, authenticated resolves. Also backfills the previous merge's changelog placeholder (tag 030). |
