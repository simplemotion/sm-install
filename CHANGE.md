...
|---------|------|------|--------|-------|
| (auto) | — | 2026-08-19 | Greg Gowans | **Ask for 100 releases when resolving a tag, because the default of 30 is a deadline the shared channels are already approaching.** The channel repos hold releases for every package at once - `sm-develop` currently has 49 across 17 packages - and the tag scan requested `/releases` with no `per_page`, so it saw the newest 30 and took the first match. Once a package's newest release falls past that boundary the scan finds nothing, and the installer exits with `No <channel> build of <package> is published yet`. That message reads like the package was never released, not like a page-size bug, so it would be diagnosed as a publishing failure. Measured rather than predicted: on 2026-08-19 the newest `sm-mcp-v` tag sat at **index 17 of 30**. Comfortable today, and it only moves one way. Both scans in `sm-install.sh` and the equivalent in `sm-install.ps1`. `releases/latest` and `releases/tags/<tag>` are untouched - they return a single object and take no page size. Found while wiring `sm-<binary> --update`, which makes this scan run far more often than a first install ever did. `bash -n` clean, `.ps1` parses under pwsh. |
| (auto) | — | 2026-08-18 | Greg Gowans | **Accept sm-ci's new identity when verifying provenance, which had silently broken every install of a post-migration build.** `sm-ci` moved from `simplemotion/sm-ci` to `3400-0000-SM-Software/3400-9991-SM-CI`. Attestations are signed by the reusable workflow, so the certificate SAN moved with it, but the identity pin in `sm-install.sh` and `sm-install.ps1` did not. **Every build cut after the migration therefore failed provenance on every channel, for every package**, with `Provenance verification failed (cosign rejected the bundle)` after the checksum had already passed. Found installing `sm-mcp-xero` `sm-mcp-v0.1.1-develop-006`. The comment directly above that line already warned that pinning the wrong identity *never matched and always rejected the bundle*; the migration reintroduced exactly that. **Both identities are accepted, deliberately.** Releases already published in the channels were signed under the old identity and must stay installable - `sm-mcp-v0.1.0-develop-249` and `-develop-001` both carry `simplemotion/sm-ci`, while `-develop-006` carries the new one, so the cutover falls between 17 and 18 August. A hard swap would have traded one broken set for another. Drop the old alternative once every published release has been re-cut. **Verified with cosign against real bundles before proposing**, not reasoned about: the alternation returns `Verified OK` for both an old-identity release and a new-identity one, and the unpatched pin rejects the new one. Provenance remains fully enforced - the check is against the identity that actually signed the artefact. `--certificate-github-workflow-repository` still binds the source repo separately and is unchanged. **Also backfills the tag stream**, which the inlined gate blocked on: `-023`, `-024` and `-025` were tagged by earlier merges but their rows were left as `(auto)`, so three rows are keyed to their tags with the hash and UTC date read from the tagged commit. Pre-existing drift, unrelated to the fix, but the gate is right to hold the PR until it is cleared. |
