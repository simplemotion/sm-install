...
|---------|------|------|--------|-------|
| (auto) | — | 2026-08-15 | Greg Gowans | **Strip markdown from the Hash and Date cells in the develop-stream check.** Mirrors `simplemotion/sm-pr`, where the bug was found: the Version key already tolerated code spans, so a row written entirely in backticks matched on its key and then failed on its Hash, reporting `` Hash is `a3cfe46`, git says a3cfe46 `` — **the same SHA, called drift**. This fork runs the step **hard**, not advisory, so here it would have **failed PRs** rather than merely warning; this repo's own changelog writes hashes bare, which is the only reason it never fired. Surfaced in `9001-9001-SM-Executive-Template`, where all 7 rows warned. **Lands before the canonical fix on purpose**: `sm-pr`'s `inline-drift` gate compares this file against canonical `main` and fails a canonical PR whose shared steps this fork does not yet carry, so the fork goes first and is briefly ahead — the same ordering as `-020`, and self-resolving on the next merge. Verified with `inline-drift`'s own extract-and-diff over all 8 contract steps: no drift against the canonical branch. |
